...
<div class="form-box bg-white mt-4" id="women_Ethnic_Wear_Blouse_details">
      ...
      <div class="form-box-content p-3 ">
        ...
        <div class="row"><div class="col-md-3"><label><b>Closure Type : </b></label></div><div class="col-md-9"><div class="mb-3"><select class="form-control mb-3 selectpicker" data-placeholder="Select Closure Type" id="closure_type" name="closure_type" required><option value="">Select Closure Type</option><?php $exp=explode(',',$result[0]->Closure_type);
			for($i=0;$i<count($exp);$i++){ ?><option value="<?php echo $exp[$i];?>" ><?php echo $exp[$i];?></option><?php }?></select></div></div></div>
        <div class="row">
            <div class="col-md-3">
            <label><b>Fabric :</b></label></div><div class="col-md-9"><input type="text"  class="form-control mb-3" name="fabric" id="fabric" placeholder="Ex Cotton Bizo" required/>
            </div></div>
        <div class="row"><div class="col-md-3"><label><b>Neck Type : </b></label></div><div class="col-md-9"><div class="mb-3"><select class="form-control mb-3 selectpicker" data-placeholder="Select Neck Type" id="neck" name="neck" required><option value="">Select Neck Type</option><?php $exp=explode(',',$result[0]->Neck);
			for($i=0;$i<count($exp);$i++){ ?><option value="<?php echo $exp[$i];?>" ><?php echo $exp[$i];?></option><?php }?></select></div></div></div>
